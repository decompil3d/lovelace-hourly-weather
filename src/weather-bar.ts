...
export class WeatherBar extends LitElement {
  ...
  render() {
    ...
    let compact_layout_css: TemplateResult | null = null;
    compact_layout_css = this.getCompactLayoutCSS(this.compact_layout);

    return html`
      <div class="main">
        ${colorStyles ?? null} ${compact_layout_css ?? null}
  ...
  private getCompactLayoutCSS(compact: boolean): TemplateResult | null {
    ...
    return html`<style>
      ...
      .axes {
        margin-top: ${compact ? 0 : 5}px;
        ${compact ? unsafeCSS('line-height: 1.3;') : ''}
      }
      .bar-block {
        display: ${compact ? 'block' : 'inline-grid'};
      }
      .bar-block-bottom {
